...
- **Step 3 (Reconcile):** Safaricom posts to `/webhooks/mpesa/callback`; parse and mark invoice `status='paid'`. A non-zero `ResultCode` (insufficient balance, cancelled, timed out, wrong PIN) is recorded instead of dropped — `invoices.last_stk_result` / `last_stk_result_at`, mapped from the raw code by `mpesa.describeResultCode()`. This never touches `invoices.status`, since `agent.js`'s reminder ladder pattern-matches that column exactly; a failed push leaves the invoice in its normal reminder cycle.
